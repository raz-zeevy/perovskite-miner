...
def plot_full_fields(sorted_df, max_length=10, percentile = 0.95):
    """
    Parameters:
    - most_important_df: *Sorted* DataFrame to plot.
    - max_length: Maximum length for x-axis labels. Default is 10.
    ...
    fig = px.bar(sorted_df,
                 x=truncated_labels_with_index,
                 y=sorted_df.values,
                 title="Data sparsity in the database per field",
                 labels={"x": "Field", "y": "% of existing values"})
    ...
    fig.add_annotation(
        ...
        y=max(sorted_df.values) * 0.9,
        # 90% of the max y value for visibility
        text=f"<b>Fields below {per} full:\n{count_above_t}</b>",
        font=font,
        showarrow=False
    )
    ...
    fig.add_annotation(
        ...
        # roughly in the middle of the second group
        y=max(sorted_df.values) * 0.9,
        text=f"<b>Fields above {per} full:\n{count_below_t}</b>",
        font=font,
        showarrow=False
    )
    # Display the plot
...
if __name__ == '__main__':
    ...
    # plot_missing_fields(fields_by_missing_freq, max_length=15)
    plot_full_fields(fields_by_full_freq, max_length=15)
